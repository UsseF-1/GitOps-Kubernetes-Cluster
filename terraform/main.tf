...
resource "aws_iam_role_policy_attachment" "nodes_eks_policy" {
...
# 7. EKS Cluster - The Kubernetes Brain
...
# 8. Node Group - EC2 Servers for Containers
...
# 11. NAT Gateway (for private subnets to access internet)
...
# 12. Private Route Tables (for private subnets)
...
# Actual resources (VPC, subnets, EKS, ECR)
# Additional policy for nodes to join cluster
resource "aws_iam_role_policy_attachment" "nodes_cluster_policy" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKSClusterPolicy"
  role       = aws_iam_role.eks_nodes.name
}
